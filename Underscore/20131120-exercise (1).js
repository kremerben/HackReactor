...
return destination;
  ...
  _.memoize = function(func) {
    ...
    // or try var args = Array.prototype.slice.apply(arguments, 1);
    //var args = func.prototype.slice.apply(arguments, 1);
  var result;
  return function() {
    // check if the arguments are already used
    // should "arguments" be "args"? 
    // should "args" be "arguments"? 
    var prop = args.apply(this,arguments);
    if (result.hasOwnProperty(prop)) {
          return result[prop]; 
    } else {
        return result[prop] = func.apply(this, arguments);
      }
  }
};

  _.memoizeCHEAT = function(func) {
return func;
};
  ...
  _.delayd = function(func, wait) {
    var args = Array.prototype.slice.call(arguments, 2);
    var callFunc = func.apply(args);
      return setTimeout(callFunc, wait);
};

  _.delayxxx = function(func, wait) {
    var args = Array.prototype.slice.call(arguments, 2);
  ...
  _.shuffle = function(array) {
    // version without a copy of array
    var shuffledArray = [];
    while (array.length>0) {
      shuffledArray.push(array.splice(Math.floor(Math.random()*array.length)));
    }
    return shuffledArray;
  };
