...
//this version wsa working fine for all tests
  ...
  _.uniq = function(array, test, iterator) {
    ...
    for (var i = 0; i < newArray.length; i++) {
      for (var j = 0; j < newArray.length; j++) {
        if (i === j) {} else if (newArray[i] === newArray[j]) {
          newArray.splice(i, 1);
        }
      }
    }
  ...
  _.memoize = function(func) {
    ...
    return function() {
      if (func.hasOwnProperty(args)) {
        ...
        // func.args = func.apply(null, arguments); //also works
      }
      return func.args;
    };
  };
...
//TEST never worked correctly - should be ok
...
//Still incomplete
  _.sortBy = function(collection, iterator) {
    ...
    if (typeof iterator === "string") {
      //  console.log("iteraotor is a  string" + iterator);
      returnArray = collection.sort(function(a, b) {
  ...
  _.intersections = function() {
    var sets = Array.prototype.slice.call(arguments, 0);
    var allElements = _.flatten(arguments);
    return _.uniq(_.filter(allElements, function(num) {
      var isInAllSets = _.every(sets, function(set) {
        return (_.contains(set, num));
      });
      return isInAllSets;
    }));
  };
  ...
  _.intersection = function() {
    var sets = Array.prototype.slice.call(arguments, 0);
    var allElements = _.flatten(arguments[0]);
    return _.uniq(_.filter(allElements, function(num) {
      var isInAllSets = _.every(sets, function(set) {
        return (_.contains(set, num));
      });
  ...
  _.difference = function(array) {
    var args = Array.prototype.slice.call(arguments, 1);
        console.log(array);
    // console.log(args); //[Array[3], Array[2]]
    var testArray = _.flatten(args);
    // console.log(testArray);// ["moe", "curly", "larry", "moe", "groucho"]
    var answer = [];

    answer = _.reject(array, function(num) {
      var asdf = _.every(testArray, function(indArgs) {
        // console.log(indArgs);
        // console.log(_.contains(indArgs, num));
        return (_.contains(indArgs, num));
      });
      if (asdf) {
        // console.log(num + "---");
        return num;
